...
import { View, Text, Button, WebView, Image, ImageBackground, ScrollView, StyleSheet, TouchableOpacity } from 'react-native'
			...
			<View style={styles.container}>
				...
				<ImageBackground style={{ width, height: 100 }} source={{ uri: imgUri }}>
					<View style={[reset.pd20]}>
						<Text style={[reset.cw, reset.fs14, reset.lh30]}>{typeList[cur].title}</Text>
						<Text style={[reset.cw, reset.fs12, reset.lh20]}>{typeList[cur].tip}</Text>
					</View>
				</ImageBackground>
				...
				<ScrollView style={{ width }}>
					{MainStore.books.map((item, i) => {
						...
						return (
							<View key={i} style={[reset.mb5, reset.bgw, reset.pd10, { width }]}>
								...
								<View style={[reset.row, { width: width - 20 }]}>
									...
									<View style={[reset.ml10, reset.br1, reset.bc]}>
										<Text style={[reset.fs14]}>{title}</Text>
										<Text style={[reset.fs12, reset.mt5, reset.lh20, reset.tip]}>{author.map(item => item.name + ' ')}</Text>
										<Text style={[reset.fs12, reset.lh20, reset.tip]}>{highlightTags.map(item => item.name + ' ')}</Text>
										<Text numberOfLines={2} style={[reset.fs12, reset.lh20, reset.tip, { width: width - 170 }]}>
											{editorHighlight}
										</Text>
									</View>
									<View
								...
								<View style={[reset.mt5, { width: width - 20 }]}>
									<Text style={[reset.br2, reset.lh30, reset.bgg, reset.fs12, reset.pl10, reset.tip]}>
