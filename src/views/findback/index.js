...
@observer
export default class Findback extends React.Component {
		...
		return (
			<Container style={styles.container}>
				...
				<Content style={{paddingLeft: 30, paddingRight: 30,paddingBottom:20,}}>
                    ...
                    <View><Text style={styles.title}>找回密码</Text></View>
	...
	async _getRegist() {
		...
		if (loading) return
		if (text.length !== 11) {
			Alert.alert('找回密码', '请输入正确的手机号码')
		} else if (verification_code.length !== 6) {
			Alert.alert('找回密码', '请输入正确的验证码')
		} else if (password.length < 6) {
			Alert.alert('找回密码', '请输入长度大于6位的新密码')
		} else {
			this.setState({ loading: true })
			const res = await UserStore.userFindback({ username: text, password, verification_code })
			this.setState({ loading: false })
			Alert.alert('找回密码', res.msg ? res.msg : '错误', [{
				text: '确定', onPress: () => {
					if (res.code === 1) {
